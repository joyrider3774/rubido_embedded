...
"""Convert the skin images in assets to the flash data headers the game includes.
  ...
  assets/skins  -> source/rubido_embedded/images

The full screen images and the menu words become run length encoded headers (<name>_RLE565.h,
see png2rle565.py), the peg tiles a raw RGB565 little endian header (<name>_RGB565_LE.h, see
png2rgb565.py): the game draws a tile out of the middle of it.
...
SOURCE_DIR = os.path.join(ROOT, "source", "rubido_embedded")

# every skin has to have these, the game includes all of them
IMAGES = png2rle565.RLE_IMAGES + ["peg"]
