...
The seven raw RGB565 full screen images (32KB each) take more flash than the smaller
devices have, so they are stored compressed and drawn with drawImageRLE() in helperfuncs.cpp.
The menu words are compressed as well, with drawImageRLETransparent() their COLOR_TRANSPARENT
(0x005F) pixels are skipped: the Gamebuino META's flash does not have room for them raw.
...
defaults to ../assets/skins and ../source/rubido_embedded/images next to this script
"""
import os
import sys
...
def main():
    ...
    images_dir = sys.argv[2] if len(sys.argv) > 2 else os.path.join(here, "..", "source", "rubido_embedded", "images")
